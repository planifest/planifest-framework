...
| **attribution.txt** | A file placed alongside each external skill's `SKILL.md` containing: licence type, copyright holder, source URL, and any attribution text required by the original licence. |
...
| **external skill** | An open-source skill sourced from outside the Planifest framework, curated into `planifest-framework/external-skills/` with a verified permissive licence and attribution record. |
...
| **pause.md** | A file written to `plan/current/pause.md` on explicit human command, capturing the current phase, active task, last completed artifact, and in-progress state to enable exact-point resume. |
| **permissive licence** | A software licence that allows use, modification, and redistribution with minimal restrictions. Accepted: MIT, Apache 2.0, ISC, BSD-2-Clause, BSD-3-Clause. Excluded: GPL, AGPL, unknown. |
