...
- **Detector 2 (P4 diff review):** performed at P4 by a fresh-context subagent (maker-checker, independent of the agent that made the edits), reviewing the full orchestrator diff line-by-line against the current repo. Result: 12 of 13 removed content areas CONFIRMED intact at their canonical target with zero loss (Fast Path, Change Pipeline, Scope Lock Challenge mechanics, Telemetry, Agent Dispatch Standards relocation, Retrofit scan, 3 of 4 Mode Taxonomy modes, all P1-P6 phase content). One CONTENT LOSS finding: the External Anchor mode's specific underlying-mode selection mapping ("archive present → also Standard-Iterative; source only → also Retrofit; neither → Greenfield baseline") had been dropped rather than relocated - the replacement prose in both the orchestrator and discovery.template.md only gestured at "per what else is present" without stating the actual rule. Fixed per ADR-002's resolution rule (restore, never rationalise): the mapping is now stated explicitly in discovery.template.md's External Anchor subsection. A related stale pointer in spec-agent/SKILL.md (still citing "orchestrator's Adoption Modes → Retrofit scan" after that scan moved to workflows/retrofit.md) was also corrected. Full pack re-confirmed green after both fixes; orchestrator word count unaffected (8,592, fixes landed in other files).

No content was deleted without a canonical-target verification; two gaps were found and closed before removal (telemetry-standards.md and workflows/retrofit.md were both missing content their orchestrator sections claimed was "already stated elsewhere" — added first, then removed from the orchestrator). One item (reversal execute/assess mechanics) was withdrawn entirely after inspection found no actual duplication. One further gap (External Anchor mapping) was caught by the independent P4 review, not by the authoring pass - confirming the value of the dual-detector process operating as two genuinely separate checks.
