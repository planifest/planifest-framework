...
- `planifest-framework/scripts/refresh-delete-boot-files.sh`
- `planifest-framework/scripts/refresh-delete-boot-files.ps1`

Added during P5 (security): the two boot-file deletion scripts, closing a HIGH finding that the deletion allowlist had no deterministic backstop (see security-report.md).

Deviation from the P0 draft: `.claude/skills/planifest-refresh-setup/` was listed here originally but `.claude/` is gitignored in this repo (a local install target regenerated by `setup.sh`, not tracked source, see `.gitignore`). A local copy is kept there for this session's own convenience only, it is never committed. `planifest-framework/skills/` is the sole tracked source for the new skill.
