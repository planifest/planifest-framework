...
- Ship process steps:
  ...
  5. Archive `plan/current/` → `plan/archive/{feature-id}-{YYYY-MM-DD}/` with `-{n}` collision suffix if same-day re-run.
  6. Confirm archive path and PR URL to human.
...
- [ ] Archive path format is `plan/archive/{feature-id}-{YYYY-MM-DD}/` with `-{n}` collision handling.
- [ ] `.skips` is deleted during the archive step.
