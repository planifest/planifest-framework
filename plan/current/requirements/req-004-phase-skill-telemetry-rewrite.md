...
- Rewrite the "Telemetry" section in each of the 8 phase skills (`planifest-orchestrator`, `planifest-spec-agent`, `planifest-adr-agent`, `planifest-codegen-agent`, `planifest-validate-agent`, `planifest-security-agent`, `planifest-docs-agent`, `planifest-ship-agent`) so that an agent-driven emission failure triggers the immediate stop-and-ask protocol (req-003) rather than the current "skip silently if unavailable" instruction
