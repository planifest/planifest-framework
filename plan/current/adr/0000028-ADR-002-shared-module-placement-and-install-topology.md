...
- `hooks/telemetry/read-stdin.mjs` (or similarly named) for `readStdin()`. Used by both directories, placed in `telemetry/` and imported cross-directory by the two `enforcement/` callers (`check-telemetry-receipts.mjs`, `check-telemetry-failures.mjs`), since `readStdin` has no enforcement-only caller and its natural home is where most callers live. Note this is the reverse import direction from the phase-enum module below, and it is safe: `readStdin` has no caller that is ever active while `telemetry/` is absent, unlike the phase-enum case. If a future enforcement-only consumer needs a helper with no telemetry callers, that helper is placed in `enforcement/` instead. See the per-module placement table below.
...
- `hooks/enforcement/phase-enum.mjs` for the canonical phase enum, imported by both `check-telemetry-receipts.mjs` (same directory) and `resolve-phase.mjs` (`hooks/telemetry/`, cross-directory import up to `../enforcement/phase-enum.mjs`).
