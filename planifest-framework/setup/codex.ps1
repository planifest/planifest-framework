...
2. **No code without documentation.** Every component MUST have a component.md manifest and docs/ artifacts. Never produce code without its corresponding documentation.
...
7. **Update documentation after any deviation.** If implementation required deviating from the spec, plan, or design decisions, you MUST update the affected artifacts (plan/, docs/, or component.md) to reflect what was actually built. Documentation must always match reality.
...
- **To read a template:** read the file directly (e.g. `planifest-framework/templates/adr.template.md`) immediately before generating that artifact.
- **To load a phase skill:** read the corresponding skill file (e.g. `planifest-framework/skills/planifest-spec-agent/SKILL.md`) at the start of that phase — not before.
...
## Key Paths
  ...
  src/                             — component code (each with component.md)
