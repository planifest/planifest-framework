...
- Never write to `plan/current/build-log.md` — the orchestrator writes the confirmation entry, and only after the human's explicit accept/edit/reject for that specific item.
- Never smooth over a flagged inconsistency to make the draft read cleaner — surfacing the flag is the point.
