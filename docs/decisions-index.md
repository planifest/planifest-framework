...
| ADR-002 | product.yml with versionPolicy | accepted | Root `product.yml` aggregates one release version across components (`max-component-version` \| `explicit` \| `external`); single-component projects keep component.yml behaviour. **Revised 2026-08-08:** under `max-component-version`, `components[]` entries hold `{id, path}` pointers to each component's own `component.yml`, not a cached `{id, version}` — `product-version.mjs` reads the live version at derivation time, eliminating the P9 sync-drift risk a cached copy created |
