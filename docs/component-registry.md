...
**Last updated:** 0000024-declared-product-id-for-telemetry (03 Aug 2026)
...
| `planifest-framework` | Planifest Framework | component-pack | developer-tooling | active | Core standards, skills, hooks, and setup scripts enforcing the confirmed-design pipeline (v0.24.0: replaced git-path `product_id` derivation with a declared `product.yml` `id` field — no fallback, unresolvable id routes to the existing failure-marker mechanism; orchestrator P0 now hard-stops to prompt for a declared id when absent; fixed stale `emit_event` MCP tool envelope-argument documentation, which had silently broken 12 of 14 agent-driven telemetry event types since a backend rename; live-reverified and closed 0000017's previously unexecuted RCA follow-up). | [component.yml](../planifest-framework/component.yml) |
